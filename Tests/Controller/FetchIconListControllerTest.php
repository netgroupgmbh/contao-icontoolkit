<?php

/**
 * @since       09.12.2025 - 14:55
 *
 * @author      Patrick Froch <info@netgroup.de>
 *
 * @see         http://www.netgroup.de
 *
 * @copyright   NetGroup GmbH 2025
 */

declare(strict_types=1);

namespace NetGroup\IconToolkit\Tests\Controller;

use NetGroup\IconToolkit\Classes\Controller\FetchIconListController;
use NetGroup\IconToolkit\Classes\Services\Helper\IconHelper;
use PHPUnit\Framework\MockObject\MockObject;
use PHPUnit\Framework\TestCase;
use Symfony\Component\HttpFoundation\JsonResponse;

class FetchIconListControllerTest extends TestCase
{

    /**
     * @var (IconHelper&MockObject)|MockObject
     */
    private $icoHelper;


    /**
     * @var MockObject|(JsonResponse&MockObject)
     */
    private $json;


    /**
     * @var FetchIconListController
     */
    private FetchIconListController $controller;


    protected function setUp(): void
    {
        $this->icoHelper        = $this->getMockBuilder(IconHelper::class)
                                       ->disableOriginalConstructor()
                                       ->getMock();

        $this->json             = $this->getMockBuilder(JsonResponse::class)
                                       ->disableOriginalConstructor()
                                       ->getMock();

        $this->controller       = $this->getMockBuilder(FetchIconListController::class)
                                       ->setConstructorArgs([$this->icoHelper])
                                       ->onlyMethods(['json'])
                                       ->getMock();
    }

    public function testInvoke(): void
    {
        $data       = ['data'];
        $options    = ['options'];

        $this->icoHelper->expects($this->once())
                        ->method('parseIconList')
                        ->willReturn($data);

        $this->icoHelper->expects($this->once())
                        ->method('getOptions')
                        ->with($data)
                        ->willReturn($options);

        $this->controller->expects($this->once())
                         ->method('json')
                         ->with($options)
                         ->willReturn($this->json);

        $this->assertSame($this->json, $this->controller->__invoke());
    }
}
