...
then
    ...
    if [[ "$phpunitversion" == "PHPUnit 12."* ]] && [ "${VERBOSE}" == "TRUE" ]
    then
        showphpunitnotices="--display-phpunit-notices"
    fi

    # PHPUnit gobal mit composer installiert
    echo
    myecho "Führe UnitTests mit globalem PHPUnit durch"
    XDEBUG_MODE=coverage ${PHP} -d error_reporting="E_ALL & ~E_DEPRECATED & ~E_STRICT" ../../../vendor/bin/phpunit ${showphpunitnotices} --configuration ${phpunitconfig} $TESTDOX
    tmperr=$?
